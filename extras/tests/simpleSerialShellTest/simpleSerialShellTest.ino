...
testF(ShellTest, noEcho) {

    bool response = false;
    int errNo = 0;

    shell.setEchoEnabled(false);

    const char* echoCmd = "echo aWord\r";
    terminal.pressKeys(echoCmd);
    response = shell.executeIfInput();
    assertTrue(response);
    errNo = shell.lastErrNo();
    ...
    // We have nothing but the output generated by the command
    assertEqual(terminal.getline(), "aWord" COMMAND_PROMPT);

    // There are statics here, so we need to set things back
    shell.setEchoEnabled(true);
};
